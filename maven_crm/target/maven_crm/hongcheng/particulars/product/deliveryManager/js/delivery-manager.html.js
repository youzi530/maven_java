...
var deliveryManagerHTML = {
			...
			var queryHtml =
					'<div class="form-inline center-block" style="margin-left: 10px;">'+
						'<div class="row" style="margin-top: 10px;margin-right: 0px;">'+
							'<div class="col-sm-5">'+
								'<button type="button" class="btn btn-info btn-sm" onclick="deliveryManagerMain.addClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-plus"></i>添加'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="deliveryManagerMain.updClient(\'\')" style="margin-right: 0.5em;">'+
									'<i class="fa fa-pencil"></i>修改'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="deliveryManagerMain.delBatchClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-minus"></i>删除'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="deliveryManagerMain.shareApplayClient()" style="margin-right: 0.5em;">'+
							...
							'<div class="col-sm-2" style="float: right;margin-top: -10px;">'+
								'<div class="form-inline" style="float: right; margin-right: 10px;min-width:168px;text-align:right;">'+
									'<input type="button" title="重置" value="重置" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="deliveryManagerMain.resetFun()">'+
									'<input type="button" title="查询" value="查询" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="deliveryManagerMain.queryFun()">'+
									'<lable class="control-label">'+
										'<button type="button" class="btn btn-info btn-sm" title="更多条件" style="margin-bottom:4px;width:46px;height:30px;margin-left:0.5em;" onClick="deliveryManagerMain.moreFun(this)">'+
		...
		queryColumns: function(isCheckbox) {
			...
			var operation = {
					...
					"formatter": function(value,row,index) {
						...
						var str = "<a href='javascript:void(0);' disabled="+disabled+" onclick='deliveryManagerMain.updClient(\""+row.id+"\")' title='修改' style='margin-left: 10px;margin-right: 10px;'><i class='fa fa-wrench'></i></a>";
						str += "<a href='javascript:void(0);' disabled="+disabled+" onclick='deliveryManagerMain.delClient(\""+row.id+"\")' title='删除'><i class='fa fa-trash-o'></i></a>";
						return str;
					}
			};
		...
		deliveryManagerHTML: function(type,data,data1,data2,data3) {
			...
			if(type =='add'){
				...
				clientHtml += '<div class="form-group">';
				clientHtml += '<label for="pName">商品名称<span style="color: red;">*</span></label>';
				clientHtml += '<select class="form-control" id="pName" disabled >';
				for(var i=0;i<snameData.length;i++){
					clientHtml+='<option id='+snameData[i].pid+' value='+snameData[i].pName+"（编号："+snameData[i].pid+"）"+'>'+snameData[i].pName+"（编号："+snameData[i].pid+"）"+'</option>';
				}
				if(type == 'update'){
			...
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="address">收货地址<span style="color: red;">*</span></label>';
			clientHtml += '<input type="text" class="form-control" id="address" placeholder="">';
			...
			if(type == 'add'){
				...
				clientHtml += '<div class="col-sm-6">';
				clientHtml += '<div class="form-group">';
				clientHtml += '<label for="saleNum">商品数量</label>';
				clientHtml += '<input type="text" class="form-control" id="saleNum" placeholder="">';
				clientHtml += '</div></div>';

				clientHtml += '<div class="col-sm-6">';
				clientHtml += '<div class="form-group">';
				clientHtml += '<label for="salePrice">商品价格</label>';
				...
				clientHtml += '<div class="form-group">';
				clientHtml += '<label for="saleNum">商品数量</label>';
				clientHtml += '<input type="text" class="form-control" id="saleNum" placeholder="" disabled>';
				clientHtml += '</div></div>';

				clientHtml += '<div class="col-sm-6">';
				clientHtml += '<div class="form-group">';
				clientHtml += '<label for="salePrice">商品价格</label>';
				clientHtml += '<input type="text" class="form-control" id="salePrice" placeholder="" disabled>';
				clientHtml += '</div></div></div>';
			}
			...
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="clientName">收货人<span style="color: red;">*</span></label>';
			// clientHtml += '<input type="text" class="form-control" id="clientName" placeholder="">';
			clientHtml += '<select class="form-control" id="clientName">';
			for(var i=0;i<client.length;i++) {
				clientHtml += '<option value=' + client[i].clientName + "（编号：" + client[i].clientId + "）" + '>' + client[i].clientName + "（编号：" + client[i].clientId + ")" + '</option>';
			}
			clientHtml += '</select>';
			clientHtml += '</div></div>';
			...
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="username">发货人<span style="color: red;">*</span></label>';
