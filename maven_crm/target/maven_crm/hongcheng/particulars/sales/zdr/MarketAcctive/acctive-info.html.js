var acctiveInfoHTML = {
			...
			var queryHtml = 
					'<div class="form-inline center-block" style="margin-left: 10px;">'+
						'<div class="row" style="margin-top: 10px;margin-right: 0px;">'+
							'<div class="col-sm-5">'+
								'<button type="button" class="btn btn-info btn-sm" onclick="acctiveInfoMain.addClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-plus"></i>添加'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="acctiveInfoMain.updClient(\'\')" style="margin-right: 0.5em;">'+
									'<i class="fa fa-pencil"></i>修改'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="acctiveInfoMain.delBatchClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-minus"></i>删除'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="acctiveInfoMain.shareApplayClient()" style="margin-right: 0.5em;">'+
							...
							'<div class="col-sm-2" style="float: right;margin-top: -10px;">'+
								'<div class="form-inline" style="float: right; margin-right: 10px;min-width:168px;text-align:right;">'+
									'<input type="button" title="重置" value="重置" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="acctiveInfoMain.resetFun()">'+
									'<input type="button" title="查询" value="查询" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="acctiveInfoMain.queryFun()">'+
									'<lable class="control-label">'+
										'<button type="button" class="btn btn-info btn-sm" title="更多条件" style="margin-bottom:4px;width:46px;height:30px;margin-left:0.5em;" onClick="acctiveInfoMain.moreFun(this)">'+
		...
		queryColumns: function(isCheckbox) {
			...
			var operation = {
					...
					"formatter": function(value,row,index) {
						...
						var str = "<a href='javascript:void(0);' disabled="+disabled+" onclick='acctiveInfoMain.updClient(\""+row.id+"\")' title='修改' style='margin-left: 10px;margin-right: 10px;'><i class='fa fa-wrench'></i></a>";
						str += "<a href='javascript:void(0);' disabled="+disabled+" onclick='acctiveInfoMain.delClient(\""+row.id+"\")' title='删除'><i class='fa fa-trash-o'></i></a>";
						return str;
					}
			};
		...
		clientInfoHTML: function(id,type,data) {
			...
			clientHtml += '<label for="">供应商名称<span style="color: red;">*</span></label>';
	...
	clientInfoHTML2: function(type,data) {
		var clientHtml = '<div class="form" style="padding:5px;">';
		clientHtml += '<div class="row">';
		clientHtml += '<div class="col-sm-6">';
		clientHtml += '<div class="form-group">';
		clientHtml += '<label for="name">市场活动名<span style="color: red;">*</span></label>';
		clientHtml += '<input type="text" class="form-control" id="name" placeholder="">';
		clientHtml += '</div></div>';
		...
		clientHtml += '<div class="col-sm-6">';
		clientHtml += '<div class="form-group">';
		clientHtml += '<label for="address">活动地点<span style="color: red;">*</span></label>';
		clientHtml += '<input type="text" class="form-control" id="address" placeholder="">';
		clientHtml += '</div></div>';

		clientHtml += '<div class="col-sm-6">';
		clientHtml += '<div class="form-group">';
		clientHtml += '<label for="gift">活动礼品<span style="color: red;">*</span></label>';
		clientHtml += '<input type="text" class="form-control" id="gift" placeholder="">';
		clientHtml += '</div></div></div>';
			clientHtml += '<div class="row">';
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="organizer">组织者<span style="color: red;">*</span></label>';
			clientHtml += '<select class="form-control" id="organizer" >';
			...
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="partner">参与人<span style="color: red;">*</span></label>';
			clientHtml += '<input type="text" class="form-control"  id="partner" placeholder="">';
			clientHtml += '</div></div></div>';

			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<input type="hidden" disabled="disabled" class="form-control" id="mid" placeholder="">';
			clientHtml += '</div></div></div>';
