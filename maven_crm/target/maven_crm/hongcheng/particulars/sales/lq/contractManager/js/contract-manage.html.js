...
var contractManageHTML = {
			...
			var queryHtml = 
					'<div class="form-inline center-block" style="margin-left: 10px;">'+
						'<div class="row" style="margin-top: 10px;margin-right: 0px;">'+
							'<div class="col-sm-5">'+
								'<button type="button" class="btn btn-info btn-sm" onclick="contractManageMain.addClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-plus"></i>添加'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="contractManageMain.updClient(\'\')" style="margin-right: 0.5em;">'+
									'<i class="fa fa-pencil"></i>修改'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="contractManageMain.delBatchClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-minus"></i>删除'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="contractManageMain.shareApplayClient()" style="margin-right: 0.5em;">'+
							...
							'<div class="col-sm-2" style="float: right;margin-top: -10px;">'+
								'<div class="form-inline" style="float: right; margin-right: 10px;min-width:168px;text-align:right;">'+
									'<input type="button" title="重置" value="重置" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="contractManageMain.resetFun()">'+
									'<input type="button" title="查询" value="查询" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="contractManageMain.queryFun()">'+
									'<lable class="control-label">'+
										'<button type="button" class="btn btn-info btn-sm" title="更多条件" style="margin-bottom:4px;width:46px;height:30px;margin-left:0.5em;" onClick="contractManageMain.moreFun(this)">'+
		...
		queryColumns: function(isCheckbox) {
			...
			var operation = {
					...
					"formatter": function(value,row,index) {
						...
						var str = "<a href='javascript:void(0);' disabled="+disabled+" onclick='contractManageMain.updClient(\""+row.id+"\")' title='修改' style='margin-left: 10px;margin-right: 10px;'><i class='fa fa-wrench'></i></a>";
						str += "<a href='javascript:void(0);' disabled="+disabled+" onclick='contractManageMain.delClient(\""+row.id+"\")' title='删除'><i class='fa fa-trash-o'></i></a>";
						return str;
					}
			};
			...
			queryArray.push(cId);
			queryArray.push(cName);
			queryArray.push(detail);
		 ...
		 */
		contractManageHTML: function(type,data1,data2,data3) {

			var client = JSON.parse(data3);
			console.log(client)
			...
			clientHtml += '<label for="cName">合同主题(合同:客户名+编号)<span style="color: red;">*</span></label>';
			clientHtml += '<select class="form-control" id="cName">';
			for(var i=0;i<client.length;i++) {
				clientHtml += '<option value=' +"合同："+ client[i].clientName + "（编号：" + client[i].clientId + "）" + '>' +"合同："+ client[i].clientName + "（编号：" + client[i].clientId + ")" + '</option>';
			}
			if(type == 'update'){
				$("#cName").find("option[value = '"+ "合同："+client[0].clientName+"（编号："+client[0].clientId+"']").attr("selected","selected");
			}
			clientHtml += '</select>';
			clientHtml += '</div></div>';
			...
			if(type =='add'){
				clientHtml += '<div class="col-sm-6">';
				clientHtml += '<div class="form-group">';
				clientHtml += '<label for="detail">订购明细<span style="color: red;">*</span></label>';
				...
				clientHtml += '<div class="form-group">';
				clientHtml += '<label for="detail">订购明细<span style="color: red;">*</span></label>';
				clientHtml += '<input type="text" class="form-control" id="detail" placeholder="" disabled>';
				clientHtml += '</div></div></div>';
			}
			...
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="giveaway">赠品<span style="color: red;">*</span></label>';
			clientHtml += '<input type="text" class="form-control" id="giveaway" placeholder="">';
			...
			// clientHtml += '<input type="text" class="form-control" id="state" placeholder="">';
			...
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="checkTime">审核时间(默认为当前时间)</label>';
			// clientHtml += '<input type="text" class="form-control" id="checkTime" placeholder="">';

			clientHtml += '<div id="checkTime" class="input-group date form_date">';
			clientHtml += '<input class="form-control size="16" type="text" value="" id="checkTime" readonly="readonly">';
