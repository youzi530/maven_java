...
var performanceManageHTML = {
			...
			var queryHtml = 
					'<div class="form-inline center-block" style="margin-left: 10px;">'+
						'<div class="row" style="margin-top: 10px;margin-right: 0px;">'+
							'<div class="col-sm-5">'+
								'<button type="button" class="btn btn-info btn-sm" onclick="performanceManageMain.addClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-plus"></i>添加'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="performanceManageMain.updClient(\'\')" style="margin-right: 0.5em;">'+
									'<i class="fa fa-pencil"></i>修改'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="performanceManageMain.delBatchClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-minus"></i>删除'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="performanceManageMain.shareApplayClient()" style="margin-right: 0.5em;">'+
							...
							'<div class="col-sm-2" style="float: right;margin-top: -10px;">'+
								'<div class="form-inline" style="float: right; margin-right: 10px;min-width:168px;text-align:right;">'+
									'<input type="button" title="重置" value="重置" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="performanceManageMain.resetFun()">'+
									'<input type="button" title="查询" value="查询" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="performanceManageMain.queryFun()">'+
									'<lable class="control-label">'+
										'<button type="button" class="btn btn-info btn-sm" title="更多条件" style="margin-bottom:4px;width:46px;height:30px;margin-left:0.5em;" onClick="performanceManageMain.moreFun(this)">'+
		...
		queryColumns: function(isCheckbox) {
			...
			var operation = {
					...
					"formatter": function(value,row,index) {
						...
						var str = "<a href='javascript:void(0);' disabled="+disabled+" onclick='performanceManageMain.updClient(\""+row.id+"\")' title='修改' style='margin-left: 10px;margin-right: 10px;'><i class='fa fa-wrench'></i></a>";
						str += "<a href='javascript:void(0);' disabled="+disabled+" onclick='performanceManageMain.delClient(\""+row.id+"\")' title='删除'><i class='fa fa-trash-o'></i></a>";
						return str;
					}
			};
			...
			queryArray.push(contactNum);
			queryArray.push(newContactNum);
		 ...
		 */
		performanceManageHTML: function(type,data1,data2) {
			...
			// clientHtml += '<div class="col-sm-6">';
			// clientHtml += '<div class="form-group">';
			// clientHtml += '<label for="contactNum">客户联系数<span style="color: red;">*</span></label>';
			// clientHtml += '<input type="text" class="form-control" id="contactNum" placeholder="">';
			...
			// clientHtml += '<div class="col-sm-6">';
			// clientHtml += '<div class="form-group">';
			// clientHtml += '<label for="newContactNum">新开发客户数<span style="color: red;">*</span></label>';
			// clientHtml += '<input type="text" class="form-control" id="newContactNum" placeholder="">';
			// clientHtml += '</div></div>';
			
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="saleOrderNum">销售订单数<span style="color: red;">*</span></label>';
			clientHtml += '<input type="text" class="form-control" id="saleOrderNum" placeholder="">';
			...
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="contractNum">合同数</label>';
			clientHtml += '<input type="text" class="form-control" id="contractNum" placeholder="">';
			clientHtml += '</div></div>';
			
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="contractMoney">合同金额</label>';
			clientHtml += '<input type="text" class="form-control" id="contractMoney" placeholder="">';
			clientHtml += '</div></div></div>';
