var callInfoMain = {
		...
		queryTable: function() {
			...
			var queryHtml = callInfoHTML.queryHTML();
			var queryColumn = callInfoHTML.queryColumns(true);
			...
			$("#client_info #client_info_table").bootstrapTable({
			    ...
			    queryParams: function(params){
			    	$.extend(params,callInfoMain.queryTableParam);
		...
		queryFun: function() {
			...
			return callInfoMain.queryTableParam;
		},
		/**
		 * 显示/隐藏其他查询条件
		 */
		...
		clientDetail: function(id, type) {
			...
			// 修改返回所有的数据
			callInfoCtroller.clientInfoDetail(id, function(data){
				var date=data;
				callInfoCtroller.yuanxinxi(function(data) {
					console.log(data);
					var content = callInfoHTML.clientInfoHTML2(type,data);
					var modelId = "client" + type + "Model";
					var isHideBut = type == 'update' ? false : true;
					commFun.noModel(modelId, "客户详情",700,500, content, callInfoMain.submitData2, type, isHideBut);
					// callInfoMain.fillData(type);
					//这里直接把拿到的数据放进去
					callInfoMain.fillData2( type, date);
					if (type != 'detail') {
						// 初始化时间组件
		...
		addClient: function() {
			callInfoCtroller.yuanxinxi(function(data) {
				var content = callInfoHTML.clientInfoHTML2('add',data);
				commFun.noModel("clientaddModel", "添加来电管理", 700, 500, content, callInfoMain.submitData2, "add", false);
// 初始化时间组件
...
// 省市级联
		...
		updClient: function(id) {
			...
			// 	}
			// }
			callInfoMain.clientDetail(id, 'update');
		},
	...
	submitData2: function(type) {
		...
		if (type == 'add') {// 添加
			callInfoCtroller.clientInfoAdd(jsonqd, function(data){
				console.log(data);
				if (data == '[]') {
					...
					// console.log(id)
					callInfoMain.datatablesDestroy();
					var json4 = JSON.parse(data);
					// console.log(json4[0]);
					addTr4(json4[0].id,json4[0].sName,json4[0].content);
					callInfoMain.datatablesCreate();
				}
			});
		} else  if(type == 'update'){// 修改
			callInfoCtroller.clientInfoUpdate(jsonqd, function(data) {
				var josn4=JSON.parse(data);
				if (data == '') {
	...
	/**
		...
		delBatchClient: function() {
			...
			sweetAlert.warning("确定删除吗？", "", "确定", "取消", null, 5000, function(){
				...
				console.log(ids);
				callInfoCtroller.clientInfoBatchDel(ids2, function(data){
					if (data == '') {
		...
		delClient: function(id) {
			sweetAlert.warning("确定删除吗？", "", "确定", "取消", null, 5000, function(){
				callInfoCtroller.clientInfoDel(id, function(data){
					console.log(data);
					if (data == '') {
						sweetAlert.caution("温馨提示",data,"关闭",3000);
					} else {
				...
				singleButtons: [{
					...
					callback: function() {
						...
						if (selectedData.length < 1) {
							...
							callInfoCtroller.clientShareApplay(clientId, userId, function(data) {
								if (data != '') {
		...
		linkmanDetail: function(id) {
			linkmanController.linkmanDetail(id, function(data){
				...
				callInfoMain.fillLinkmanData(modelId,  data);
			});
		},
...
window.clientEvent = {
	"click .like":function(e,value,row,index){
		callInfoMain.clientDetail(row.id, 'detail');
	}
}
window.userEvent = {
	"click .like":function(e,value,row,index){
