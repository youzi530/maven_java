var userInfo = {
    ...
    addTr:function (newTr) {
        ...
        $("#linkman_info_table tbody").append($('<tr id="linkmanTr'+newTr.userId+'" data-index="0"> ' +
            ...
            '<td style="width: 80px; "><a href="javascript:void(0);" disabled="false" onClick="userInfoMain.updLinkman('+newTr.userId+')" title="修改" style="margin-left: 10px;margin-right: 10px;"><i class="fa fa-wrench"></i></a>'+del+'</td> </tr>'));
    },
    ...
    update:function (userId,tr) {
        ...
        $("#linkman_info_table #linkmanTr"+ tr.userId + " td:eq(5) span").text(tr.roleId);
        $("#linkman_info_table #linkmanTr"+ tr.userId + " td:eq(6) span").text(tr.status);
        $("#linkman_info_table #linkmanTr"+ tr.userId + " td:eq(7)").text(tr.mobile);
        $("#linkman_info_table #linkmanTr"+ tr.userId + " td:eq(8)").text(tr.email);
        $("#linkman_info_table #linkmanTr"+ tr.userId + " td:eq(9) a").html(tr.account+','+'&nbsp;&nbsp;'+tr.password);
