...
var emarketActivitiesHTML = {
			...
			var queryHtml = 
					'<div class="form-inline center-block" style="margin-left: 10px;">'+
						'<div class="row" style="margin-top: 10px;margin-right: 0px;">'+
							'<div class="col-sm-5">'+
								'<button type="button" class="btn btn-info btn-sm" onclick="emarketActivitiesMain.addClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-plus"></i>添加'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="emarketActivitiesMain.updClient(\'\')" style="margin-right: 0.5em;">'+
									'<i class="fa fa-pencil"></i>修改'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="emarketActivitiesMain.delBatchClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-minus"></i>删除'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="emarketActivitiesMain.shareApplayClient()" style="margin-right: 0.5em;">'+
							...
							'<div class="col-sm-2" style="float: right;margin-top: -10px;">'+
								'<div class="form-inline" style="float: right; margin-right: 10px;min-width:168px;text-align:right;">'+
									'<input type="button" title="重置" value="重置" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="emarketActivitiesMain.resetFun()">'+
									'<input type="button" title="查询" value="查询" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="emarketActivitiesMain.queryFun()">'+
									'<lable class="control-label">'+
										'<button type="button" class="btn btn-info btn-sm" title="更多条件" style="margin-bottom:4px;width:46px;height:30px;margin-left:0.5em;" onClick="emarketActivitiesMain.moreFun(this)">'+
		...
		queryColumns: function(isCheckbox) {
			...
			var operation = {
					...
					"formatter": function(value,row,index) {
						...
						var str = "<a href='javascript:void(0);' disabled="+disabled+" onclick='emarketActivitiesMain.updClient(\""+row.id+"\")' title='修改' style='margin-left: 10px;margin-right: 10px;'><i class='fa fa-wrench'></i></a>";
						str += "<a href='javascript:void(0);' disabled="+disabled+" onclick='emarketActivitiesMain.delClient(\""+row.id+"\")' title='删除'><i class='fa fa-trash-o'></i></a>";
						return str;
					}
			};
			...
			queryArray.push(sName);
			queryArray.push(pType);
		 ...
		 */
		emarketActivitiesHTML: function(type,data) {
			...
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="sName">供应商名称</label>';
			clientHtml += '<select class="form-control" id="sName">';
			//clientHtml += '<option value=""></option>';

			for(var i=0;i<snameData.length;i++){
				clientHtml+='<option value='+snameData[i].sName+'>'+snameData[i].sName+'</option>';
			}
			if(type == 'update'){
				$("#sName").find("option[value = '"+data[0].sName+"']").attr("selected","selected");
			}
			...
			// 	}
			// }else if(type == 'update'){
			// 	clientHtml+='<option value='+snameData[0].sName+'>'+snameData[0].sName+'</option>';
			//
			// }
			...
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="pType">商品类别<span style="color: red;">*</span></label>';
			clientHtml += '<input type="text" class="form-control" id="pType" placeholder="">';
			clientHtml += '</div></div>';
			
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="pPrice">商品价格<span style="color: red;">*</span></label>';
			clientHtml += '<input type="text" class="form-control" id="pPrice" placeholder="">';
			clientHtml += '</div></div></div>';
			...
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="pNum">商品数量<span style="color: red;">*</span></label>';
			clientHtml += '<input type="text" class="form-control" id="pNum" placeholder="">';
			clientHtml += '</div></div>';
			...
			clientHtml += '<div class="col-sm-12">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="pDescription">商品描述</label>';
			clientHtml += '<textarea class="form-control" id="pDescription" style="resize: none;"></textarea>';
			clientHtml += '</div></div></div>';
