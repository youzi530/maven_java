var costAssInfoMain = {
		...
		queryTable: function() {
			...
			var queryHtml = costAssInfoHTML.queryHTML();
			var queryColumn = costAssInfoHTML.queryColumns(true);
			...
			$("#client_info #client_info_table").bootstrapTable({
			    ...
			    queryParams: function(params){
			    	$.extend(params,costAssInfoMain.queryTableParam);
					costAssInfoMain.queryTableParam = {};
			    	return params;
			    },
			    showColumns: false,
			    showRefresh: false,
		...
		queryFun: function() {
			...
			return costAssInfoMain.queryTableParam;
		},
		/**
		 * 显示/隐藏其他查询条件
		 */
	...
	datatablesDestroy:function(){
		$('#cost_anaalysis_table').dataTable().fnDestroy();
	},

	datatablesCreate:function(){
		jQuery(function($) {
			var oTable1 = $('#cost_anaalysis_table').dataTable( {
		...
		clientDetail: function(id, type) {
			...
			// 修改返回所有的数据
			costAssInfoCtroller.clientInfoDetail(id, function(data){
				var date=data;
				costAssInfoCtroller.yuanxinxi(function(data) {
					console.log(data);
					var content = costAssInfoHTML.clientInfoHTML2(type,data);
					var modelId = "client" + type + "Model";
					var isHideBut = type == 'update' ? false : true;
					commFun.noModel(modelId, "客户详情",700,500, content, costAssInfoMain.submitData2, type, isHideBut);
					// costAssInfoMain.fillData(type);
					//这里直接把拿到的数据放进去
					costAssInfoMain.fillData2( type, date);
					if (type != 'detail') {
						// 初始化时间组件
		...
		addClient: function() {
			costAssInfoCtroller.yuanxinxi(function(data) {
				var content = costAssInfoHTML.clientInfoHTML2('add',data);
				commFun.noModel("clientaddModel", "添加渠道信息", 700, 500, content, costAssInfoMain.submitData2, "add", false);
// 初始化时间组件
...
// 省市级联
		...
		updClient: function(id) {
			...
			// 	}
			// }
			costAssInfoMain.clientDetail(id, 'update');
		},
	...
	fillData2: function(type,data) {
		...
		$("#gouzei").find("option[value = '"+data[0].sName+"']").attr("selected","selected");
		$("#id3").val(""+data[0].id+"")
	},
	...
	submitData2: function(type) {
		...
		var res = false;
		var supplier= {id:""+id+"",sName:""+sName+"",content:""+content+""};
		var jsonqd = JSON.stringify(supplier);
		if (type == 'add') {// 添加
			costAssInfoCtroller.clientInfoAdd(jsonqd, function(data){
				console.log(data);
				if (data == '[]') {
					...
					// console.log(json4[0]);
					addTr4(json4[0].id,json4[0].sName,json4[0].content);
				}
			});
		} else  if(type == 'update'){// 修改
			costAssInfoCtroller.clientInfoUpdate(jsonqd, function(data) {
				var josn4=JSON.parse(data);
				if (data == '') {
					...
					$("#client_info #client_info_table").bootstrapTable("refresh");
					res = true;
					$("#"+josn4[0].id+"gys").text(josn4[0].sName);
					$("#"+josn4[0].id+"content").text(josn4[0].content);
				}
			});
		}
	...
	/**
		...
		delBatchClient: function() {
			...
			sweetAlert.warning("确定删除吗？", "", "确定", "取消", null, 5000, function(){
				...
				console.log(ids);
				costAssInfoCtroller.clientInfoBatchDel(ids2, function(data){
					if (data == '') {
		...
		delClient: function(id) {
			sweetAlert.warning("确定删除吗？", "", "确定", "取消", null, 5000, function(){
				costAssInfoCtroller.clientInfoDel(id, function(data){
					console.log(data);
					if (data == '') {
						sweetAlert.caution("温馨提示",data,"关闭",3000);
					} else {
				...
				singleButtons: [{
					...
					callback: function() {
						...
						if (selectedData.length < 1) {
							...
							costAssInfoCtroller.clientShareApplay(clientId, userId, function(data) {
								if (data != '') {
		...
		linkmanDetail: function(id) {
			linkmanController.linkmanDetail(id, function(data){
				...
				costAssInfoMain.fillLinkmanData(modelId,  data);
			});
		},
...
window.clientEvent = {
	"click .like":function(e,value,row,index){
		costAssInfoMain.clientDetail(row.id, 'detail');
	}
}
window.userEvent = {
	"click .like":function(e,value,row,index){
