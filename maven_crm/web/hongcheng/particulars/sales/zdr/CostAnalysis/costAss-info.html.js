var costAssInfoHTML = {
			...
			var queryHtml = 
					'<div class="form-inline center-block" style="margin-left: 10px;">'+
						'<div class="row" style="margin-top: 10px;margin-right: 0px;">'+
							'<div class="col-sm-5">'+
								'<button type="button" class="btn btn-info btn-sm" onclick="costAssInfoMain.addClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-plus"></i>添加'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="costAssInfoMain.updClient(\'\')" style="margin-right: 0.5em;">'+
									'<i class="fa fa-pencil"></i>修改'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="costAssInfoMain.delBatchClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-minus"></i>删除'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="costAssInfoMain.shareApplayClient()" style="margin-right: 0.5em;">'+
							...
							'<div class="col-sm-2" style="float: right;margin-top: -10px;">'+
								'<div class="form-inline" style="float: right; margin-right: 10px;min-width:168px;text-align:right;">'+
									'<input type="button" title="重置" value="重置" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="callInfoMain.resetFun()">'+
									'<input type="button" title="查询" value="查询" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="callInfoMain.queryFun()">'+
									'<lable class="control-label">'+
										'<button type="button" class="btn btn-info btn-sm" title="更多条件" style="margin-bottom:4px;width:46px;height:30px;margin-left:0.5em;" onClick="callInfoMain.moreFun(this)">'+
		...
		queryColumns: function(isCheckbox) {
			...
			var operation = {
					...
					"formatter": function(value,row,index) {
						...
						var str = "<a href='javascript:void(0);' disabled="+disabled+" onclick='costAssInfoMain.updClient(\""+row.id+"\")' title='修改' style='margin-left: 10px;margin-right: 10px;'><i class='fa fa-wrench'></i></a>";
						str += "<a href='javascript:void(0);' disabled="+disabled+" onclick='costAssInfoMain.delClient(\""+row.id+"\")' title='删除'><i class='fa fa-trash-o'></i></a>";
						return str;
					}
			};
	...
	clientInfoHTML2: function(type,data) {
		...
		clientHtml += '<div class="col-sm-6">';
		clientHtml += '<div class="form-group">';
		clientHtml += '<label for="content">内容<span style="color: red;">*</span></label>';
		clientHtml += '<input type="text" class="form-control" id="content" placeholder="">';
		clientHtml += '</div></div>';

		clientHtml += '<div class="row">';
		clientHtml += '<div class="col-sm-6">';
		clientHtml += '<div class="form-group">';
		clientHtml += '<label for="s">供应商名称<span style="color: red;">*</span></label>';
		clientHtml += '<div id="小小" class="form-group">';
		clientHtml += '<select class="form-control" id="gouzei" style="float: left; width: inherit; margin-right: 10px;">'
		for(var i=0;i<data.length;i++) {
			clientHtml+='<option value='+data[i].realName+'>'+data[i].realName+'</option>';
		}
		if(type == 'update'){
		...
		clientHtml += '<div class="col-sm-6">';
		clientHtml += '<div class="form-group">';
		clientHtml += '<input type="hidden" class="form-control" id="id3" placeholder="">';
		clientHtml += '</div></div></div>';
