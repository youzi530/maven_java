var fileInfoMain = {
		...
		queryTable: function() {
			...
			var queryHtml = fileInfoHTML.queryHTML();
			var queryColumn = fileInfoHTML.queryColumns(true);
			...
			$("#client_info #client_info_table").bootstrapTable({
			    ...
			    queryParams: function(params){
			    	$.extend(params,fileInfoMain.queryTableParam);
					fileInfoMain.queryTableParam = {};
			    	return params;
			    },
			    showColumns: false,
			    showRefresh: false,
		...
		queryFun: function() {
			...
			return fileInfoMain.queryTableParam;
		},
		/**
		 * 显示/隐藏其他查询条件
		 */
		...
		clientDetail: function(id, type) {
			...
			// 修改返回所有的数据
			fileInfoCtroller.clientInfoDetail(id, function(data){
					console.log(data);
					var content = fileInfoHTML.clientInfoHTML2(type);
					var modelId = "client" + type + "Model";

					commFun.noModel("knowledgeDetailModel", "文件描述",700,500, content, fileInfoMain.submitData2, type, false);
					// callInfoMain.fillData(type);
					//这里直接把拿到的数据放进去
					fileInfoMain.fillData2( type, data);
					if (type != 'detail') {
						// 初始化时间组件
	...
	datatablesCreate:function(){
		 ...
		 */
		addClient: function() {
				var content = fileInfoHTML.clientInfoHTML2('add');
				commFun.noModel("knowledgeDetailModel", "添加文件",700,500, content, function(){

					if ($("#knowledgeDetailModel #descrption").val() == "")
					{
						sweetAlert.caution("温馨提示","文件描述不允许为空！")
						return;
					}
					// if ($("#knowledgeDetailModel #register").val() == ""){
					// 	sweetAlert.caution("温馨提示","上传时间不允许为空！")
					// 	return;
					// }
					if($("#knowledgeDetailModel #file").val()==""){
						sweetAlert.caution("温馨提示","上传文件不允许为空！")
						return;
					}

					var descrption= $("#knowledgeDetailModel #descrption").val();
					// var time=$("#knowledgeDetailModel #register").val();
					var time = new Date();
					...
					var name=$("#knowledgeDetailModel #file").val();
					// console.log(name);
					var name1 = name.substring(12,name.length);
					// console.log(na
					alert(descrption +":" + time)
					//$("#knowledgeForm").submit()
					var options = {
						url : "add.file?descrption=" + descrption + "&time=" + time,
						dataType:"json",
						type:"POST",

						success : function(result) {
							console.log(result.password);
							if(result.password=="用户存在"){
								sweetAlert.caution("温馨提示", "已存在");
							}
							else{
								console.log(result);
								sweetAlert.caution("温馨提示", "添加成功");
								$.noModel("closeModel", "knowledgeDetailModel");
								addTr4(result.password,name1,descrption,time,name1)
							}
...
// 初始化时间组件
				$('#knowledgeDetailModel #registerTime').datetimepicker({
					language: 'zh-CN',
					format: "yyyy年mm月dd日",// 日期格式
					minView: "month",// 精确视图，这样只有年月日，没有时分秒
					autoclose: true,// 是否自动关闭
...
// 省市级联
		...
		updClient: function(id) {
			...
			// 	}
			// }
			fileInfoMain.clientDetail(id, 'update');
		},
	...
	fillData2: function(type,data) {
        ...
        console.log(data);
        $("#knowledgeDetailModel #register").val(""+data.date+"");
        $("#knowledgeDetailModel #descrption").val(""+data.description+"");
        $("#knowledgeDetailModel #id").val(""+data.id+"");
	...
	submitData2: function(type) {
		var description = $("#knowledgeDetailModel #descrption").val();
		var date=$("#knowledgeDetailModel #register").val();
		var id=$("#knowledgeDetailModel #id").val();
		...
		if (date == '') {
			sweetAlert.caution("温馨提示", "时间不允许为空！");
			return false;
		}

		var res = false;
		var supplier= {id:""+id+"",fileName:"",description:""+description+"",date:""+date+"",uri:""};
		var jsonqd = JSON.stringify(supplier);
		fileInfoCtroller.clientInfoUpdate(jsonqd, function(data) {
				var josn4=JSON.parse(data);
				if (data == '') {
					...
					$("#client_info #client_info_table").bootstrapTable("refresh");
					res = true;
					$("#"+josn4.id+"description").text(josn4.description);
					$("#"+josn4.id+"date").text(josn4.date);
				}
			});
		return res;
	},
	...
	/**
		...
		delBatchClient: function() {
			...
			sweetAlert.warning("确定删除吗？", "", "确定", "取消", null, 5000, function(){
				...
				// });
				// console.log(ids);
				fileInfoCtroller.clientInfoBatchDel(ids2, function(data){
					if (data == '') {
		...
		delClient: function(id) {
			sweetAlert.warning("确定删除吗？", "", "确定", "取消", null, 5000, function(){
				fileInfoCtroller.clientInfoDel(id, function(data){
					console.log(data);
					if (data == '') {
						sweetAlert.caution("温馨提示",data,"关闭",3000);
					} else {
				...
				singleButtons: [{
					...
					callback: function() {
						...
						if (selectedData.length < 1) {
							...
							fileInfoCtroller.clientShareApplay(clientId, userId, function(data) {
								if (data != '') {
		...
		linkmanDetail: function(id) {
			linkmanController.linkmanDetail(id, function(data){
				...
				fileInfoMain.fillLinkmanData(modelId,  data);
			});
		},
...
window.clientEvent = {
	"click .like":function(e,value,row,index){
		fileInfoMain.clientDetail(row.id, 'detail');
	}
}
window.userEvent = {
	"click .like":function(e,value,row,index){
