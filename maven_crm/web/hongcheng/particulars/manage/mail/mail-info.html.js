var mailInfoHTML = {
			...
			var queryHtml = 
					'<div class="form-inline center-block" style="margin-left: 10px;">'+
						'<div class="row" style="margin-top: 10px;margin-right: 0px;">'+
							'<div class="col-sm-5">'+
								'<button type="button" class="btn btn-info btn-sm" onclick="mailInfoMain.addClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-plus"></i>添加'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="mailInfoMain.updClient(\'\')" style="margin-right: 0.5em;">'+
									'<i class="fa fa-pencil"></i>修改'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="mailInfoMain.delBatchClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-minus"></i>删除'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="mailInfoMain.shareApplayClient()" style="margin-right: 0.5em;">'+
							...
							'<div class="col-sm-2" style="float: right;margin-top: -10px;">'+
								'<div class="form-inline" style="float: right; margin-right: 10px;min-width:168px;text-align:right;">'+
									'<input type="button" title="重置" value="重置" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="mailInfoMain.resetFun()">'+
									'<input type="button" title="查询" value="查询" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="mailInfoMain.queryFun()">'+
		...
		queryColumns: function(isCheckbox) {
			...
			var operation = {
					...
					"formatter": function(value,row,index) {
						...
						var str = "<a href='javascript:void(0);' disabled="+disabled+" onclick='mailInfoMain.updClient(\""+row.id+"\")' title='修改' style='margin-left: 10px;margin-right: 10px;'><i class='fa fa-wrench'></i></a>";
						str += "<a href='javascript:void(0);' disabled="+disabled+" onclick='mailInfoMain.delClient(\""+row.id+"\")' title='删除'><i class='fa fa-trash-o'></i></a>";
						return str;
					}
			};
		 ...
		 */
		clientInfoHTML: function(type,data) {
			...
			clientHtml += '<label for="s">收件人<span style="color: red;">*</span></label>';
			clientHtml += '<div id="小小" class="form-group">';
			clientHtml += '<select class="form-control" id="receiveMan" style="float: left; width: inherit; margin-right: 10px;">'
			for(var i=0;i<data.length;i++) {
				clientHtml+='<option value='+data[i].userId+'>'+data[i].realName+'</option>';
			}
			clientHtml += '</select></div></div></div></div>';
			clientHtml += '<div class="row">';
			clientHtml += '<div class="col-sm-6">';
			clientHtml += '<div class="form-group">';
			clientHtml += '<label for="s">发送内容<span style="color: red;">*</span></label>';
			clientHtml += '<textarea  id="content" class="" placeholder="">';
