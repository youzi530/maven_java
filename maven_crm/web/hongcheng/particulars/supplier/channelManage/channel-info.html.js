var channelInfoHTML = {
			...
			var queryHtml = 
					'<div class="form-inline center-block" style="margin-left: 10px;">'+
						'<div class="row" style="margin-top: 10px;margin-right: 0px;">'+
							'<div class="col-sm-5">'+
								'<button type="button" class="btn btn-info btn-sm" onclick="channelInfoMain.addClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-plus"></i>添加'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="channelInfoMain.updClient(\'\')" style="margin-right: 0.5em;">'+
									'<i class="fa fa-pencil"></i>修改'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="channelInfoMain.delBatchClient()" style="margin-right: 0.5em;">'+
									'<i class="fa fa-minus"></i>删除'+
								'</button>'+
								'<button type="button" class="btn btn-info btn-sm" onclick="channelInfoMain.shareApplayClient()" style="margin-right: 0.5em;">'+
							...
							'<div class="col-sm-2" style="float: right;margin-top: -10px;">'+
								'<div class="form-inline" style="float: right; margin-right: 10px;min-width:168px;text-align:right;">'+
									'<input type="button" title="重置" value="重置" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="channelInfoMain.resetFun()">'+
									'<input type="button" title="查询" value="查询" class="btn navbar-btn btn-info btn-sm" style="margin-right:0.5em;" onClick="channelInfoMain.queryFun()">'+
									'<lable class="control-label">'+
										'<button type="button" class="btn btn-info btn-sm" title="更多条件" style="margin-bottom:4px;width:46px;height:30px;margin-left:0.5em;" onClick="channelInfoMain.moreFun(this)">'+
		...
		queryColumns: function(isCheckbox) {
			...
			var operation = {
					...
					"formatter": function(value,row,index) {
						...
						var str = "<a href='javascript:void(0);' disabled="+disabled+" onclick='channelInfoMain.updClient(\""+row.id+"\")' title='修改' style='margin-left: 10px;margin-right: 10px;'><i class='fa fa-wrench'></i></a>";
						str += "<a href='javascript:void(0);' disabled="+disabled+" onclick='channelInfoMain.delClient(\""+row.id+"\")' title='删除'><i class='fa fa-trash-o'></i></a>";
						return str;
					}
			};
		...
		clientInfoHTML: function(id,type,data) {
			...
			for(var i=0;i<data.length;i++){
				clientHtml+='<option value='+data[i].realName+'>'+data[i].realName+"  工号"+data[i].userId+'</option>';
			}
	...
	clientInfoHTML2: function(type) {
		...
		clientHtml += '<div class="form-group">';
		clientHtml += '<label for="qudaoName">供应商名称<span style="color: red;">*</span></label>';
		clientHtml += '<input type="text" class="form-control" id="qudaoName" placeholder="">';
		clientHtml += '<input type="hidden" id="clientId">';
		clientHtml += '<input type="hidden" id="status">';
		clientHtml += '</div></div>';
		...
		clientHtml += '<label for="distpicker">联系员工</label>';
		clientHtml += '<div id="小小" class="form-group">';
		clientHtml += '<select class="form-control" id="dagongzai"  style="float: left; width: inherit; margin-right: 10px;"></select>';
		// clientHtml += '<select class="form-control" id="city" style="float: left; width: inherit; margin-right: 10px;"></select>';
		// clientHtml += '<select class="form-control" id="town" style="float: left; width: inherit;"></select>';
