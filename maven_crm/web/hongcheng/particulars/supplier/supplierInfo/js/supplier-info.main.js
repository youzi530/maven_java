var supplierInfoMain = {
		...
		queryTable: function() {
			...
			var queryHtml = supplierInfoHTML.queryHTML();
			var queryColumn = supplierInfoHTML.queryColumns(true);
			...
			$("#client_info #client_info_table").bootstrapTable({
			    ...
			    queryParams: function(params){
			    	$.extend(params,supplierInfoMain.queryTableParam);
					supplierInfoMain.queryTableParam = {};
			    	return params;
			    },
			    showColumns: false,
			    showRefresh: false,
		...
		queryFun: function() {
			...
			return supplierInfoMain.queryTableParam;
		},
		/**
		 * 显示/隐藏其他查询条件
		 */
	...
	datatablesCreate:function(){
		...
		clientDetail: function(id, type) {
			supplierInfoCtroller.clientInfoDetail(id, function(data){
				console.log(data);
				//判断是否存在，返回通过主键获得的数据
				if (data.msg == 0) {
				...
				// var content = supplierInfoHTML.clientInfoHTML2(type);
				var modelId = "client" + type + "Model";
				var isHideBut = type == 'update' ? false : true;
				var content = supplierInfoHTML.clientInfoHTML2(type,data);
				//将这条数据展示在页面上\
				//这里通过jquery可以拿到所有数据，
				commFun.noModel(modelId, "供应商xinxi",700,500, content, supplierInfoMain.submitData2, type, isHideBut);
		 ...
		 */
		addClient: function() {
			var content = supplierInfoHTML.clientInfoHTML('add');
			commFun.noModel("clientaddModel", "添加供应商信息",700,500, content, supplierInfoMain.submitData, "add" , false);
			// 初始化时间组件
		...
		updClient: function(id) {
			...
			supplierInfoMain.clientDetail(id, 'update');
		},
	...
	/**
		...
		submitData: function(type) {
			...
			console.log(sName );
			var  nature= $("#" + modelId + " #nature").val();
			console.log(nature);
			var mainProduct = $("#" + modelId + " #mainProduct").val();
			console.log(mainProduct);
			var phone = $("#" + modelId + " #phone").val();
			console.log(phone);
			var license = $("#" + modelId + " #license").val();
			console.log(license);
			if (sName == '') {
		...
		var supplier= {sid:null,sName:""+sName+"",nature:""+nature+"",mainProduct:""+mainProduct+"",phone:""+phone+"",license:""+license+""};
			var jsonsup = JSON.stringify(supplier);
			var res = false;
			if (type == 'add') {// 添加
				supplierInfoCtroller.clientInfoAdd(jsonsup, function(data){
					if (data == '[]') {
						...
						res = true;
						// console.log(data);
						supplierInfoMain.datatablesDestroy();

						var json4 = JSON.parse(data);
						// console.log(json4[0]);
						addTr(json4[0].sid,json4[0].sName,json4[0].nature,json4[0].mainProduct,json4[0].phone,json4[0].license);
						supplierInfoMain.datatablesCreate();
					}
				});
			} else {// 修改
				var id = $("#" + modelId + " #clientId").val();
				supplierInfoCtroller.clientInfoUpdate(id, clientName, workAddress, mainPhone, zipCode, email,
	...
	submitData2: function(type) {
		...
		var jsonsup = JSON.stringify(supplier);
		var res = false;
		supplierInfoCtroller.clientInfoUpdate(jsonsup, function(data){
			var json4=JSON.parse(data);
			console.log(json4)
			if (data != '') {
				supplierInfoMain.queryTableParam = {};
		...
		delBatchClient: function() {
			...
			sweetAlert.warning("确定删除吗？", "", "确定", "取消", null, 5000, function(){
				...
				console.log("ids");
				supplierInfoCtroller.clientInfoBatchDel(ids, function(data){
					if (data == '') {
		...
		delClient: function(id) {
			sweetAlert.warning("确定删除吗？", "", "确定", "取消", null, 5000, function(){
				supplierInfoCtroller.clientInfoDel(id,function(data){
					console.log(data);
					if (data == '') {
						sweetAlert.caution("温馨提示",data,"关闭",3000);
					} else {
				...
				singleButtons: [{
					...
					callback: function() {
						...
						if (selectedData.length < 1) {
							...
							supplierInfoCtroller.clientShareApplay(clientId, userId, function(data) {
								if (data != '') {
		...
		linkmanDetail: function(id) {
			linkmanController.linkmanDetail(id, function(data){
				...
				supplierInfoMain.fillLinkmanData(modelId,  data);
			});
		},
...
window.clientEvent = {
	"click .like":function(e,value,row,index){
		supplierInfoMain.clientDetail(row.id, 'detail');
	}
}
window.userEvent = {
	"click .like":function(e,value,row,index){
